...
- **Validación de Componentes**: Consulta `mcp__shadcn__get_item` antes de escribir componentes de UI para confirmar firmas y props correctos.
  ...
  <design_standards>
    - **Alineación con DESIGN.md y Ejemplos de Layout (OBLIGATORIO)**: Debes leer obligatoriamente el archivo `.openspec/DESIGN.md` y aplicar de forma estricta sus tokens de diseño (paleta de colores HSL, radio de bordes, tipografía, etc.) en los estilos CSS de globals.css y en las clases de Tailwind de tus componentes. **Además, debes leer y basarte obligatoriamente en las maquetas y ejemplos HTML interactivos copiados en el directorio `.openspec/design-assets/<brandId>/` (ej: `preview.html`, `preview-dark.html`) para copiar los layouts exactos (sidebars, grids, headers), las sombras, transiciones y efectos hover propios de la marca en tus componentes de `src/`.**
    - **Estética Premium**: Sigue el skill `shadcn-templates` (Sección 3.5). No crees páginas vacías o minimalistas; usa los layouts y estilos completos extraídos de los ejemplos interactivos locales. **Siempre incluye iconos interactivos (de `lucide-react`) al lado de textos importantes, headers, toggles y botones para dar dinamismo a la interfaz**.
