...
```json
{
  ...
  "design": {
    "system": "shadcn-zinc-default",
    "tokens": "inherited from .opencode/templates/nextjs-shadcn/src/app/globals.css (zinc native)"
  },
  "stack": {
...
- **NO busques iconos uno por uno**: PROHIBIDO usar `lucide-icons_fuzzy_search_icons` de forma repetitiva en un loop para cada icono. Usa la herramienta optimizada por lotes `sdd_validate_lucide_icons_batch` (tool `sdd_lucide_validate_lucide_icons_batch`) pasándole todo el arreglo de nombres de iconos que desees usar en una sola llamada.
- **NO embabes código TSX en el contrato.** El contrato es declarativo (qué, no cómo).
...
[spec-writer]  → write .openspec/specs/.../contract.json (rellenando plantilla; design=shadcn-zinc-default)
