---
name: omd:init
description: "프로젝트 .opencode/DESIGN.md를 부트스트랩 — 실제 기업 레퍼런스 중 컨텍스트 매칭으로 추천하고 선택된 레퍼런스의 톤&매너를 보존한 variation을 생성. DESIGN.md 부재 상태에서의 UI 작업 또는 '디자인 시스템 세팅', 'set up our design system', 「デザインシステムを作って」, 「建立設計系統」류의 요청에 트리거."
---
...
프로젝트에 DESIGN.md를 세팅. 레퍼런스 톤&매너는 **preserve**하고, 사용자 프로젝트 맥락은 controlled-vocabulary delta_set으로만 반영.
...
레퍼런스 확정 직후, 어떻게 적용할지 묻는다. 먼저 `.opencode/DESIGN.md`가
이미 있는지 확인하고, **Claude Code 채널이면 AskUserQuestion 1개** (타 채널은
prose)로:

- .opencode/DESIGN.md **없음**:
  1. **프로젝트 디자인 시스템으로 설정 (추천)** — `<id>` 톤을 보존한 변형본을
     `.opencode/DESIGN.md`로 부트스트랩 (Phase 4~7 전체 진행)
  2. **참고용으로만 저장** — 변경 없음. `<id>` 원본을
     `design-references/<id>.md`로 복사하고 종료. "나중에 프로젝트에 적용하려면
     다시 omd:init을 불러주세요" 안내. (변형 생성·셤 설치 안 함)
- .opencode/DESIGN.md **이미 있음** (option description에 그 파일의 §0/§1 요약
  한 줄을 보여줄 것):
...
`.opencode/DESIGN.md`가 이미 있으면 (Phase 3.5에서 이미 "교체"를 확인받은
상태 — 여기서 다시 묻지 않는다):
1. Read로 내용 확인
2. Bash로 `mv .opencode/DESIGN.md .opencode/DESIGN_DEPRECATED.md`
3. `.opencode/DESIGN_DEPRECATED.md` 최상단에 다음 헤더 prepend (Edit 툴):
   ```html
   <!-- omd:deprecated
   replaced_at: <ISO-8601 today>
   replaced_by: .opencode/DESIGN.md (bootstrapped from <id>)
...
Write 툴로 `.opencode/DESIGN.md` emit.

## Phase 6 — Shim 설치 생략

이 프로젝트에서는 `AGENTS.md`, `CLAUDE.md`, `.cursor/rules` 등의 shim 파일 설치를 생략하고 오직 `DESIGN.md`만 생성 및 관리합니다. `omd:sync` 스킬 실행은 호출하지 않거나 생략합니다.
...
- 생성된 파일 목록 (.opencode/DESIGN.md)
- .opencode/DESIGN_DEPRECATED.md 있으면 언급
- 다음 스텝: `.opencode/DESIGN.md`를 기반으로 UI 작업 시작 또는 `omd:harness`로 전체 surface 디자인 또는 `omd:remember`로 선호 추가 로깅

예시:
```
✓ .opencode/DESIGN.md created (based on banksalad, 한국 핀테크 랜딩 맥락)
  - primary hue 유지 (#04c584 그대로)
  - radius 유지 (2px 시스템)
  - voice hints: 데이터 어드바이저 톤

Next:
  - UI 작업 시작 → DESIGN.md 규칙을 준수하여 진행
