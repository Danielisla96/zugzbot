...
Los valores de configuración local anulan los valores predeterminados remotos. Consulte [precedencia de configuración](https://opencode.ai/docs/config#precedence-order) para obtener más detalles.
...
| oauth Objeto OAuth configuración de autenticación. Consulte la sección a continuación. |
| timeout Número Tiempo de espera en ms para recuperar herramientas del servidor MCP. El valor predeterminado es 5000 (5 segundos). |
...
1. Utilice **Registro dinámico de cliente (RFC 7591)** si el servidor lo admite.
1. Almacene tokens de forma segura para futuras solicitudes
...
Si el servidor requiere autenticación, OpenCode le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `opencode mcp auth `.
...
1. En su [configuración del agente](https://opencode.ai/docs/agents#tools), habilite el servidor MCP como herramienta.
...
Alternativamente, puede agregar algo como esto a su [AGENTS.md](https://opencode.ai/docs/rules/).
