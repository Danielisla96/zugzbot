...
export default tool({
  ...
  async execute(args, context) {
    ...
    // 1. Detectar cambio activo del sdd-lock si no se provee
    if (!changeName) {
    ...
    // Secciones requeridas y sus expresiones regulares de validación
    ...
    if (missingSections.length > 0) {
      return JSON.stringify({
        ...
        missingSections,
        message: `❌ VALIDACIÓN FALLIDA: Faltan las siguientes secciones requeridas en el plano técnico:\n${missingSections.map(s => `  - ${s}`).join("\n")}\n\nPor favor, pide a @sdd-planner que complete todas las secciones requeridas.`
      }, null, 2);
    }

    // Comprobar si hay placeholders obvios sin completar
    const placeholders = [
      /\[nombre-cambio\]/i,
    ...
    if (detectedPlaceholders.length > 0) {
      return JSON.stringify({
        ...
        message: `❌ VALIDACIÓN FALLIDA: El spec.md contiene marcadores de posición (placeholders) que no han sido completados:\n${detectedPlaceholders.map(p => `  - ${p}`).join("\n")}\n\nPor favor, reemplaza estos valores con las especificaciones reales antes de avanzar.`
      }, null, 2);
    }

    // Validar existencia de al menos un escenario Given-When-Then
    ...
    if (!hasScenario || !hasGWT) {
      return JSON.stringify({
        ...
        message: `❌ VALIDACIÓN FALLIDA: Se requiere al menos un escenario estructurado 'Scenario:' con pasos Given-When-Then (Dado-Cuando-Entonces) para habilitar las pruebas automáticas en la Fase 2.`
      }, null, 2);
    }
    ...
    return JSON.stringify({
      status: "APPROVED",
      changeName,
      message: `✅ VALIDACIÓN EXITOSA: El plano técnico 'specs/spec.md' para '${changeName}' cumple al 100% con los contratos de calidad de la metodología SDD. Está listo para ser implementado en la Fase 2.`
    }, null, 2);
  }
})