...
const PluginTuiSidebar = async (api) => {
    api.slots.register({
        order: 100,
        slots: {
            sidebar_content(_ctx, props) {
                const [sessionIds, setSessionIds] = createSignal([props.session_id]);
                const getZugzbotVersion = () => {
                ...
                function extractAgentName(messages, sessionInfo, sessionId) {
                ...
                function getMetrics(currentSessionIds) {
                    let totalCost = 0;
                    let totalInput = 0;
                    let totalOutput = 0;
                    const agentMap = {};
                ...
                const [metrics, setMetrics] = createSignal(getMetrics([props.session_id]));
                const [sddProgress, setSddProgress] = createSignal(getSddProgress());
            ...
            <box gap={0} paddingTop={1} paddingLeft={1}>
              {logoLines.map((line, idx) => (<text fg={getLineColor(idx)}>
                  {line}
                </text>))}
            ...
            {sddProgress() ? (<box gap={0} paddingLeft={1} paddingTop={1} paddingBottom={0}>
                <text fg="#FF7300">
                ...
                {(() => {
                            ...
                            return (<text fg={tddColor} paddingTop={1}>
                      {`TDD: RED[${redMark}] GRN[${greenMark}] REF[${refactorMark}]`}
                    </text>);
                        })()}

                {(sddProgress()?.git?.branch && sddProgress()?.git?.branch !== "—") ? (<text fg={sddProgress()?.git?.workingTreeClean ? api.theme.current.success : "#FF7300"}>
            ...
            <box gap={0} paddingLeft={1} paddingTop={1} paddingBottom={0}>
              {metrics().agents.map((agent) => (<text fg={agent.isSubagent ? api.theme.current.textMuted : api.theme.current.text}>
...
export default { id: "plugin_tui", tui: PluginTuiSidebar };
